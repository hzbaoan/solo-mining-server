...
- `name`: `mainnet`, `testnet`, or `regtest`
...
- Variable difficulty is enabled by default.
- `mining.suggest_difficulty` and passwords in the form `d=<number>` set the starting share difficulty for that miner.
- When a miner supplies `suggest_difficulty` or `d=<number>`, that value becomes the starting difficulty and resets the vardiff window. After that the miner follows the normal vardiff retarget rules around `difficulty.target_time`, including both upward and downward adjustments.
- Version rolling is supported through `mining.configure` and `mining.set_version_mask`.
- If a miner omits `version-rolling.mask`, the server defaults it to `ffffffff` before intersecting with the server mask.
- Unsupported `mining.configure` extensions are returned as `false` in the negotiation result.
...
- `p2p_fast_peer` enables an additional Bitcoin P2P listener for early clean-job updates ahead of the full template refresh.
...
- ZMQ signals do not replace `longpoll`; they schedule immediate direct refreshes so reconnecting miners can receive a fresh full template without waiting for a periodic poll.
- P2P fast announcements publish the clean job immediately, and the later ZMQ or `longpoll` update fills in the full template without forcing another clean transition unless the tip actually changed again.
