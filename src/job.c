...
static const char *refresh_source_name(enum solo_refresh_source source)
{
    ...
    case SOLO_REFRESH_ZMQ_RAWBLOCK:
        return "zmq_rawblock";
    case SOLO_REFRESH_P2P_FAST_FOLLOWUP:
        return "p2p_fast_followup";
    ...
    uint32_t block_nbits)
{
    ...
    if (update_cb)
        update_cb(job, true);
    applied = true;
    solo_job_request_refresh(SOLO_REFRESH_P2P_FAST_FOLLOWUP);
    return applied;
}
...
sds solo_job_build_coinbase1(const struct solo_job *job, const char *worker_tag)
{
    ...
    sdsfree(message);
    if (sdslen(job->coinbaseaux_bin) > 0 && pack_oppush(&p, &left, job->coinbaseaux_bin, sdslen(job->coinbaseaux_bin)) < 0)
        return NULL;
...
static int apply_gbt_result(json_t *result, enum solo_refresh_source source)
{
    ...
    if (!clean_jobs && settings.keep_old_jobs == 0)
        clean_jobs = true;
    if (existing) {
        solo_job_release(existing);
        existing = NULL;
...
static void *refresh_main(void *unused)
{
    ...
    while (refresh_running) {
        ...
        if (result != NULL) {
            (void)apply_gbt_result(result, source);
            json_decref(result);
        } else {
            update_refresh_status(false);
...
static void *longpoll_main(void *unused)
{
    ...
    while (__atomic_load_n(&longpoll_running, __ATOMIC_ACQUIRE)) {
        ...
        if (result != NULL) {
            (void)apply_gbt_result(result, SOLO_REFRESH_STARTUP);
            json_decref(result);
            continue;
        }
